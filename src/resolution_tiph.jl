    ...
    for p in 1:n
        for t in 1:n
            if new_M[p,t]==2
                ...
                if black 
                    pb, new_M, new_cv = noircir(p,t,new_M,new_cv)
                    if pb
                        return true, M, cases_vides
                    end
                end
...
function heuristicSolve()
    ...
    while cases_vides >0
        for i in 1:n           # on parcourt pour trouver les cases blanches
            for j in 1:n
                if M[i,j] == 2
                    white = true
                    for k in 1:n 
            ...
            while not black && j <= n-1
                if M[i,j] == 2
                    if G[i,j] == G[i,j+1]
                        if M[i,j+1] >0
                            black = true 
                            pb, M, cases_vides = noircir(i,j,M,cases_vides)
                            if pb #on teste si cela a créer un probleme, si oui on noircit l'autre
                                pb, M , cases_vides = noircir(i,j+1,M,cases_vides)
                            end
                    elseif G[i,j] == G[i+1,j]
                        black = true 
                        pb, M, cases_vides = noircir(i,j,M,cases_vides)
                        if pb 
                            pb, M, cases_vides= noircir(i+1,j,M,cases_vides)
                        end
                    end
                end 
                j = j+2
